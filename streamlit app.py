...
def show_pizza(player_row, stat_cols, df_filtered, role_name, lightmode=False, toppings=True):
    ...
    # alt text always boxed, even no toppings
    ...
    fig, ax = baker.make_pizza(
        ...
        value_colors=text_colors,  # keep values visible in a box
    ...
    # For "no toppings" modes: hide the header texts (you asked for only stats)
    if toppings:
        ...
        fig.text(0.5, 0.952, f"{club} | {nationality} | {age} | {season}", ha='center', va='top', fontsize=15,
...
def plot_role_leaderboard(df_filtered, role_name, role_stats):
    ...
    blue_gradient = ["#112052","#1A3474","#1F2C7B","#274097","#3457A4","#4370BA","#578BC8","#6FA7D6","#8EC3E0","#B2DFFC"]
    ...
    labels = [f"{row['Player']} • {row['RoleScore']:.1f} • {row.get('Age','?')} • {row.get('Squad','?')} • {int(row.get('Mins',0)):,} mins" for _, row in top.iterrows()]
    fig = go.Figure([
        go.Bar(
            x=top['RoleScore'],
...
def show_top_players_by_stat(df, tidy_label, stat_col):
    ...
    blue_gradient = ["#112052","#1A3474","#1F2C7B","#274097","#3457A4","#4370BA","#6FA7D6","#8EC3E0","#B2DFFC"][::-1]
    bar_colors = blue_gradient[-len(top):][::-1]
    def lum(c):
        r,g,b = [x*255 for x in mcolors.hex2color(c)]
        return 0.299*r + 0.587*g + 0.114*b
    bar_text_colors = ['white' if lum(c) < 150 else '#222' for c in bar_colors]
    labels = [f"{row['Player']} • {row.get(stat_col,0):.2f} • {row.get('Age','?')} • {row.get('Squad','?')} • {int(row.get('Mins',0)):,} mins" for _, row in top.iterrows()]
    fig = go.Figure([
        go.Bar(
...
def build_role_matrix_axes(ax, df, player_row, role_x, role_y):
    ...
    if not me.empty:
        ax.scatter(me['RoleScore_X'], me['RoleScore_Y'], s=60, edgecolor="#000", linewidth=1.2, color="#B80019", zorder=5)
...
BASE = st.sidebar.text_input("Base folder", r"C:\Users\willi\Data Viz\FBREF scrapes")
...
# ======================
# Dashboard mode is intentionally removed from UI per your request
MODE_ITEMS = [
    ("Similar", "1"),
    ("Percentiles", "2"),
    ("Pizza", "3"),
...
if mode == "1":
    if player_row is None or arch_choice is None:
        ...
        if other_row is not None:
            ...
            lines = [f"{other_row['Player']} (Age: {other_row.get('Age','?')}, Club: {other_row.get('Squad','?')}, Minutes: {other_row.get('Mins','?')})",
    ...
    if len(numeric_cols) >= 2:
        ...
        hover = [
            f"<b>{r['Player']}</b><br>Team: {r.get('Squad','?')}<br>Age: {r.get('Age','?')}<br>"
            f"{x_pick}: {r[x_col]:.2f}<br>{y_pick}: {r[y_col]:.2f}<br>Minutes: {int(r.get('Mins',0)):,}"
        ...
        fig = go.Figure([
            go.Scatter(
                ...
                text=dfx['Player'], hovertext=hover, hoverinfo='text'
            )
        ])
        fig.update_layout(
            ...
            plot_bgcolor=POSTER_BG, paper_bgcolor=POSTER_BG, height=800, font=dict(family=FONT_FAMILY, color="#000")
        )
        st.plotly_chart(fig, use_container_width=True,theme=None)
    else:
    ...
    hover = [
        f"<b>{r['Player']}</b><br>Team: {r.get('Squad','?')}<br>Age: {r.get('Age','?')}<br>"
        f"{role_x}: {r['RoleScore_X']:.1f}<br>{role_y}: {r['RoleScore_Y']:.1f}<br>Minutes: {int(r.get('Mins',0)):,}"
        for _, r in dfc.iterrows()
    ]
    fig = go.Figure([
        go.Scatter(
            x=dfc['RoleScore_X'], y=dfc['RoleScore_Y'], mode='markers',
            marker=dict(size=9, color=dfc['Age'], colorscale="Blues", line=dict(width=1, color="#333")),
            text=dfc['Player'], hovertext=hover, hoverinfo='text'
        )
    ])
    fig.update_layout(
